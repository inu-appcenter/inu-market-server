...
RESPONSE_CODE=$(curl -s -o /dev/null -w "%{http_code}" https://inu-market.cf/api/port)
...
then
    ...
    CURRENT_PORT=$(curl -s https://inu-market.cf/api/port)
fi
...
sudo docker pull juhwan0815/inu-market:latest
